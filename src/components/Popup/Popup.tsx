...
export const Popup = ({ position, addBuilding, removeBuilding, close }: PopupProps) => {
  const ref = useDetectClickOutside({ onTriggered: close });
  const popupPosition = useMemo(() => new Vector3(position.x, position.y + 1, position.z), [position]);

  const buildings = useStore((state) => state.buildings);

  const hasBuilding = buildings[position.x] && buildings[position.x][position.z];
